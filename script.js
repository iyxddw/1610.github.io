...
const stat = {
	...
	get accNum() {
	    ...
	    const good = (1/this.numOfNotes)*this.good*0.685
...
function loop() {
	...
	ctx.fillText(`Phigros Simulator v${_i[1].join('.')} - Code by lch\x7ah3473`, (canvas.width + canvasos.width) / 2 - lineScale * 1, canvas.height - lineScale * 18.5);
...
function calcqwq(now) {
	...
	clickEvents1.defilter(i => now >= i.time + i.duration+2000); //清除打击特效
	clickEvents2.defilter(i => now >= i.time + i.duration+2000); //清除打击特效//changed
...
function qwqdraw1(now) {
	...
	for (const i of clickEvents1) { //绘制打击特效1
		const tick = (now - i.time) / i.duration;
		ctxos.globalAlpha = 1;
		ctxos.setTransform(...imgFlip(noteScale * 9, 0, 0, noteScale * 9, i.offsetX, i.offsetY)); //缩放
	...
	ctxos.drawImage(res["ProgressBar"], (qwq[5] ? duration - timeBgm : timeBgm) / duration * 1920 - 1920, 1190);
	//绘制文字
	ctxos.resetTransform();
	ctxos.fillStyle = "#fff";
	//开头过渡动画
	...
	//绘制分数装饰
	ctxos.drawImage(res["top"], canvasos.width - canvasos.width / 1.62, canvasos.height - lineScale * 19.5, lineScale * 7, lineScale * 5);
	//绘制歌名和等级
	ctxos.globalAlpha = 1;
...
function qwqdraw3(statData) {
	...
	ctxos.fillText(stat.good, 870, 610);
